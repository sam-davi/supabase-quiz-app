...
export default async function TopFiveCategories({ team }: { team: string }) {
  const categories = await nextCategoryPageAction(team, undefined, 2, 5);
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex justify-between">Top 5 categories</CardTitle>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
